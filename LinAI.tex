...
Sei $A\in M(m\times n,K)$. Die \textbf{transponierte Matrix} $^tA\in M(n\times m,K)$ erhält man durch Spiegeln an der Diagonalen / Vertauschen von Zeilen und Spalten. Formal: Wenn $A=(a_{ij})_{i=1,\ldots,m; j=1,\ldots,n}$, dann ist $^tA=(a'_{ij})_{i=1,\ldots,n; j=1,\ldots,m}$ mit $a'_{ij}=a_{ji}$.\\
...
\end{Lemma}
\begin{Definition}
Eine Matrix $A\in M(n\times n ,K)$ heißt \herv{invertierbar}, wenn es $A'\in M(n\times n,K)$ gibt, mit $A\cdot A'=A'\cdot A=E_n$. Unter dem Isomorphismus $M(n \times n,K)\rightarrow \emph{Hom}_K(K^n,K^n), A\mapsto (x \mapsto A\cdot x)$ entsprechen die invertierbaren Matrizen den Vektorraum-Isomorphismen.
\end{Definition}
\begin{Lemma}
...
\end{Lemma}
\section{Verschiedenes aus den Übungszetteln}
Link zu den Übungszetteln und der Vorlesungshomepage:\\
\texttt{http://www.math.uni-bonn.de/ag/topo/Lineare\_Algebra\_WS12/}
\begin{Satz}[Übung 4.1]
Sei $f: K \rightarrow R$ ein Homomorphismus von Ringen. Falls K ein Körper ist, ist f automatisch injektiv oder gleich $0$ (das nennt man auch den \emph{trivialen Ringhomomorphismus}). Es gibt keinen nicht-trivialen Ringhomomorphismus von einem Körper nach $\Z$.
\end{Satz}
\begin{Definition}[Übung 5.1]
Sei R ein beliebiger Ring mit $1$. Nenne einen Ringhomomorphismus \herv{unital}, wenn er das $1$-Element auf das $1$-Element schickt.
\end{Definition}
\begin{Satz}[Übung 5.1]
Für jeden Ring R gibt es genau einen unitalen Ringhomomorphismus $\Z \rightarrow R$.
\end{Satz}
\begin{Satz}[Übung 7.3]\label{Aufgabe7-3}
Sei L ein Körper und $K\subset L$ ein Teilkörper. L wird mit seiner Addition und der Abbildung $K\times L \rightarrow L$ gegeben durch die Körpermultiplikation zu einem K-Vektorraum.
\end{Satz}
\begin{Satz}[Übung 7.5]
Sei K ein endlicher Körper. Dann existieren eine Primzahl p und eine natürliche Zahl n, so dass K genau $p^n$ Elemente hat.
\end{Satz}
\begin{Definition}[Übung 8.5]
Sei K ein Körper. Eine Matrix $A\in M(n \times n, K)$ heißt \herv{symmetrisch}, wenn sie gleich ihrer eigenen Transponierten ist: $A =\presuper t A$. Sie heißt \herv{schiefsymmetrisch} (oder \herv{alternierend}),
falls $A = −\presuper t A$.
\end{Definition}
\begin{Satz}[Übung 8.5]
\begin{enumerate}
\renewcommand{\labelenumi}{\emph{(\alph{enumi})}}
\item Die symmetrischen Matrizen bilden einen Untervektorraum $\operatorname{Sym}(n,K)$ von $M(n \times n, K)$.
\item Die schiefsymmetrischen Matrizen bilden einen Untervektorraum $\operatorname{Alt}(n, K)$ von $M (n \times n; K)$ .
\item Sei nun char(K) $\neq$ 2. Dann $M(n \times n, K) = \operatorname{Sym}(n, K) \oplus \operatorname{Alt}(n, K)$.
\end{enumerate}
\end{Satz}
